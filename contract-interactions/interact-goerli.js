...
async function borrowBook(title, wallet) {
    const borrowTransaction = await bookLibrary.connect(wallet).borrowBook(title);
...
const interactionWithContract = async function () {
    const provider = new providers_1.InfuraProvider("goerli", process.env.INFURA_PROJECT_ID);
    const wallet1 = new ethers_1.Wallet(process.env.GOERLI_WALLET_1_PK || "", provider);
    const wallet2 = new ethers_1.Wallet(process.env.GOERLI_WALLET_2_PK || "", provider);
    bookLibrary = new ethers_1.Contract(process.env.BOOK_LIBRARY_GOERLI_ADDRESS || "", BookLibraryArtifact.abi, wallet1);
    // add first book
